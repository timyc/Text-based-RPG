            ...
            connection.query('SELECT * FROM users WHERE username = ?', [username], function(error, results, fields) {
                if (results.length > 0 && bcrypt.compareSync(password, results[0].password)) {
                    connection.query('DELETE FROM sessions WHERE LOWER(data) REGEXP \',"username":"' + (username.replace(/[^a-z0-9]/gi, '')).toLowerCase() + '","access"\'', function(error) {
                        ...
                        connection.query('UPDATE users SET lastonline = NOW(), ipaddress = ? WHERE id = ?', [(req.headers['x-forwarded-for'] || req.connection.remoteAddress), req.session.userid], function(error, results) {
                            if (error) {
                                req.session.destroy();
                    ...
                    res.status(401).send('Incorrect password/username! Try again in 5 seconds.');
                }
            });
        } else {
            res.status(401).send('Incorrect password/username! Try again in 5 seconds.');
...
exports.register = function(req, res) {
    ...
    var password = req.body.password;
    var hashedPass = bcrypt.hashSync(password, 6);
    ...
    const secretKey = "6LeI8KAUAAAAAKmwVFi7xWxh-3tlWDWYnJ_e_uur";
        ...
        if (username && password && email && (gender == '1' || gender == '2') && password.length >= 6) {
            connection.query('SELECT * FROM users WHERE username = ? OR email = ?', [username, email], function(error, results, fields) {
                if (error) {
                    ...
                    res.status(400).send('Username or Email already exists! Try again in 5 seconds.');
                } else {
                    connection.query('INSERT INTO users SET ?', allTogether, function(error, results) {
                        if (error) {
