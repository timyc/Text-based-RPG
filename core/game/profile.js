...
exports.profile = function(req, res) {
	if (req.session.loggedIn) {
		var getProfile = req.body.x;
		var gName = '-';
		var gRank = 'N/A';
		var gString = 'Not in a guild';
			...
			if (error) {
				...
				connection.query('SELECT guilds.name AS gname, users_guilds.rank AS grank FROM guilds, users_guilds WHERE guilds.id = users_guilds.guild AND users_guilds.user = ?', [results[0].id], function(error, results2) {
					...
					connection.query('SELECT achievements.name AS aname, achievements.description AS adesc, achievements.src AS asrc FROM achievements, users_achievements WHERE achievements.id = users_achievements.achievement AND users_achievements.user = ?', [results[0].id], function(error, results3) {
						var rankRank;
						var repRank;
						var pGender = results[0].gender == 1 ? '<i class="fa fa-mars text-primary"></i>' : '<i class="fa fa-venus text-danger"></i>';
						switch (true) {
						...
						res.write('<div class="col"><div class="card text-white" style="margin-right:10px;border:none; background-color: rgba(0, 0, 0, 0);"><div class="card-body"><div class="text-center">' + repRank + '</div><img src="img/avatars/' + results[0].avatar + '.png" width="200" height="200" /></div></div></div>');
						...
						res.write('</div><br /><hr><br /><center>Achievements</center>');
						if (results3.length > 0) {
							res.write('<div class="row"><div class="col"><center>');
							for (x = 0; x < results3.length; x++) {
								res.write('<img src="' + results3[x].asrc + '" width="80" title="' + results3[x].adesc + '" />');
							}
							res.write('</center></div></div>');
						} else {
							res.write('<center>None</center>');
						}
						res.end();
					});
				});
	...
	if (req.session.loggedIn) {
		connection.query('SELECT level FROM users WHERE id = ?', [req.session.userid], function(error, results) {
			if (results[0].level >= 1000) {
				...
				var uploadDest = multer.diskStorage({
				...
				upload(req, res, function(err) {
					...
					var theImage = fs.readFileSync(avatarsDirectory + '/' + req.session.userid + '.png').toString('hex', 0, 4);
					if (!checkMagicNumbers(theImage)) {
						fs.unlinkSync(avatarsDirectory + '/' + req.session.userid + '.png');
						connection.query('UPDATE users SET avatar = 0 WHERE id = ?', [req.session.userid, req.session.userid], function(error, results) {
							if (error) {
						...
						connection.query('UPDATE users SET avatar = ? WHERE id = ?', [req.session.userid, req.session.userid], function(error, results) {
							if (error) {
								return res.end('Critical database error.');
							}
