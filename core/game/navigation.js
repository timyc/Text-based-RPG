...
exports.inventory = function(req, res) {
	var menu = '<center><a href="javascript:inventory(0, 0, 0);" class="text-light">[Artifacts]</a> <a href="javascript:inventory(0, 0, 1);" class="text-light">[Misc]</a> <a href="javascript:commands(&quot;inventoryCount&quot;);" class="text-light">[Space check]</a></center>';
	var heaM = 0;
	var atkM = 0;
	var defM = 0;
	var accM = 0;
			...
			if (error) {
				...
				for (x = 0; x < results.length; x++) {
					...
					if (results[x].equipped == 1) {
						res.write('<tr><td><a rel="popover" data-trigger="hover click" data-placement="bottom" title="' + items[results[x].item - 1].description + '"><font color="#' + color + '">' + items[results[x].item - 1].name + '</font></a></td><td>' + items[results[x].item - 1].health + '</td><td>' + items[results[x].item - 1].attack + '</td><td>' + items[results[x].item - 1].defense + '</td><td>' + items[results[x].item - 1].accuracy + '</td><td>' + items[results[x].item - 1].evasion + '</td><td><a href="javascript:inventory(' + results[x].id + ',1,0);" class="text-light">[U]</a></td></tr>');
					}
				}
				...
				res.write('<center>No artifacts found.</center>[BREAK]');
				res.write(numeral(results[0].health).format('0,0') + ' (' + numeral(heaM).format('0,0') + ')[BREAK]' + numeral(results[0].attack).format('0,0') + ' (' + numeral(atkM).format('0,0') + ')[BREAK]' + numeral(results[0].defense).format('0,0') + ' (' + numeral(defM).format('0,0') + ')[BREAK]' + ((results[0].accuracy + 5000) / 100) + '% (' + ((accM + 5000) / 100) + '%)[BREAK]' + (results[0].evasion / 100) + '% (' + ((accM + 5000) / 100) + '%)');
				res.end();
			}
		});
			...
			connection.query('SELECT users_items.*, items.type FROM users_items, items WHERE users_items.user = ? AND users_items.item = items.id AND items.type = "misc" ORDER BY items.id', [req.session.userid], function(error, results) {
				if (error) {
					...
					for (x = 0; x < results.length; x++) {
						...
						if (results[x].equipped == 0) {
							res.write('<tr><td><a rel="popover" data-trigger="hover click" data-placement="bottom" title="' + items[results[x].item - 1].description + '"><font color="#' + color + '">' + items[results[x].item - 1].name + '</font></a></td><td>x' + results[x].quantity + '</td><td><a href="javascript:inventory(' + results[x].id + ',2,0);" class="text-danger">[D]</a></td></tr>');
						}
					}
					res.write('</tbody></table>[BREAK]');
					res.end();
				} else {
					res.write('<center>No misc items found.</center>[BREAK]');
					res.end();
				}
			});
				...
				connection.beginTransaction(function(error) {
					...
					connection.query('SELECT * FROM users_items WHERE user = ? AND id = ? FOR UPDATE', [req.session.userid, req.body.x], function(error, results) {
						if (error) {
							...
							connection.query('UPDATE users_items SET equipped = NOT equipped WHERE id = ?', [req.body.x], function(error, results) {
								if (error) {
									connection.rollback();
									return res.end('server error.');
								} else {
									connection.query('SELECT users_items.user AS user, users_items.equipped AS equipped, users_items.item AS item, items.health AS health, items.attack AS attack, items.defense AS defense, items.accuracy AS accuracy, items.evasion AS evasion FROM users_items, items WHERE users_items.user = ? AND users_items.equipped = 1 AND users_items.item = items.id', [req.session.userid], function(error, results) {
										if (error) {
											...
											if (results.length > req.session.equipslots) {
												connection.query('UPDATE users_items SET equipped = NOT equipped WHERE id = ?', [req.body.x], function(error, results) {
													if (error) {
														connection.rollback();
														return res.end('server error.');
							...
							connection.rollback();
							return res.end('<h3 class="text-danger">ITEM NOT IN INVENTORY</h3>[BREAK]' + numeral(req.session.hea).format('0,0') + ' (' + numeral(req.session.heaM).format('0,0') + ')[BREAK]' + numeral(req.session.atk).format('0,0') + ' (' + numeral(req.session.atkM).format('0,0') + ')[BREAK]' + numeral(req.session.def).format('0,0') + ' (' + numeral(req.session.defM).format('0,0') + ')[BREAK]' + ((req.session.acc + 5000) / 100) + '% (' + ((req.session.accM + 5000) / 100) + '%)[BREAK]' + (req.session.eva / 100) + '% (' + ((req.session.accM + 5000) / 100) + '%)[BREAK]');
						}
					});
				});
			});
			...
			connection.getConnection(function(error, connection) {
				...
				connection.beginTransaction(function(error) {
					...
					connection.query('SELECT * FROM users_items WHERE user = ? AND id = ? AND equipped = 0 FOR UPDATE', [req.session.userid, req.body.x], function(error, results) {
						...
						if (results.length == 1) {
							connection.query('DELETE FROM users_items WHERE user = ? AND id = ?', [req.session.userid, req.body.x], function(error, results) {
								if (error) {
									connection.rollback();
									return res.end('server error.');
								}
		...
		connection.query('SELECT location, locationx, locationy FROM users WHERE id = ?', [req.session.userid], function(error, result) {
			...
			if (req.body.x == 1) {
				if (result[0].locationy < locations[result[0].location - 1].distancey) {
					connection.query('UPDATE users SET locationy = locationy + 1 WHERE id = ?', [req.session.userid], function(error, results) {
						if (error) {
							return res.end('error');
						} else {
							result[0].locationy++;
							var arrayIt = JSON.parse(JSON.stringify((locPlaces)));
							...
							if (funcP2.length > 0) {
								if ((funcP2[0].function).split(' ')[0] == 'enter') {
									var funcP3 = (funcP2[0].function).split(' ')[1];
									placeString = '<input type="button" onclick="toPlace(' + funcP3 + ')" value="[Portal to ' + locations[funcP3 - 1].name + ']" class="blankBtn"><br />';
								} else if ((funcP2[0].function).split(' ')[0] == 'shop') {
									var funcP3 = (funcP2[0].function).split(' ')[1];
									placeString = '<input type="button" onclick="openpage(7, ' + funcP3 + ')" value="[Access ' + shops[funcP3 - 1].name + ']" class="blankBtn"><br />';
								}
							}
				...
				if (result[0].locationx < locations[result[0].location - 1].distancex) {
					connection.query('UPDATE users SET locationx = locationx + 1 WHERE id = ?', [req.session.userid], function(error, results) {
						if (error) {
							return res.end('error');
						} else {
							result[0].locationx++;
	...
	if (req.session.loggedIn) {
		connection.query('SELECT level FROM users WHERE id = ?', [req.session.userid], function(error, userData) {
			...
			connection.query('SELECT * FROM locations WHERE level <= ?', [userData[0].level], function(error, results) {
				...
				res.write('<br /><p class="text-center">Pay 1 salite to teleport to the (0, 0) start of a location.</p>');
...
exports.milestones = function(req, res) {
	if (req.session.loggedIn) {
		connection.query('SELECT gold, credits, mithril FROM users WHERE id = ?', [req.session.userid], function(error, uResults) {
			var milestoneCurrency = '[BREAK]' + numeral(uResults[0].gold).format('0,0') + '[BREAK]' + numeral(uResults[0].credits).format('0,0') + '[BREAK]' + numeral(uResults[0].mithril).format('0,0');
			var menu = '<center><a href="javascript:milestones(1)" class="text-light">[Incomplete]</a> <a href="javascript:milestones(2)" class="text-light">[Completed]</a></center><br />';
			if (req.body.x == 1) {
				connection.query('SELECT * FROM milestones WHERE id NOT IN (SELECT milestone FROM users_milestones WHERE user = ?)', [req.session.userid], function(error, results) {
					...
					res.write(menu);
					res.write('<center><small>Click "Complete" if you have reached the milestone for rewards!</small></center><br />');
					res.write('<div class="container-fluid"><div class="row" style="margin-left:20px;padding-left:10px;">');
					if (results.length > 0) {
						for (var y = 0; y < results.length; y++) {
							res.write('<div class="col-sm-5" style="min-width:11rem;"><div class="card text-white bg-dark" style="height:135px;width:11rem;margin:unset;border-radius:0.25rem;border-color:unset;margin-bottom:10px;border: 2px solid white"><div class="card-body" style="padding-top:unset;padding-bottom:unset;padding:0.25rem;"><h6 class="card-title text-center">' + results[y].name + '</h6><p class="card-text text-center">' + results[y].description + '</p><center><input type="button" class="btn btn-light" onclick="milestones(3, ' + results[y].id + ')" value="Complete"></center></div></div></div>');
						}
					} else {
						res.write('<center>It appears that you have completed all the milestones.</center>');
					}
				...
				connection.query('SELECT * FROM milestones WHERE id IN (SELECT milestone FROM users_milestones WHERE user = ?)', [req.session.userid], function(error, results) {
					...
					if (results.length > 0) {
						for (var y = 0; y < results.length; y++) {
							res.write('<div class="col-sm-5" style="min-width:11rem;"><div class="card text-white bg-dark" style="height:135px;width:11rem;margin:unset;border-radius:0.25rem;border-color:unset;margin-bottom:10px;border 2px solid white"><div class="card-body" style="padding-top:unset;padding-bottom:unset;padding:0.25rem;"><h6 class="card-title text-center">' + results[y].name + '</h6><p class="card-text text-center">' + results[y].description + '</p><center><input type="button" class="btn btn-dark" value="Completed" disabled></center></div></div></div>');
						}
					} else {
						res.write('<center>You have no completed milestones yet.</center>');
					}
					res.write('</div></div>' + milestoneCurrency);
					res.end();
				});
					...
					connection.query('SELECT * FROM milestones WHERE id = ?', [req.body.y], function(error, results) {
						...
						if (results.length == 1) {
							connection.query('SELECT * FROM users_milestones WHERE user = ? AND milestone = ?', [req.session.userid, req.body.y], function(error, checker) {
								...
								if (checker.length > 0) {
									return res.end(menu + '<center>You already redeemed for this milestone!</center>');
								}
								if (results[0].type == 'level') {
									connection.query('SELECT level FROM users WHERE id = ?', [req.session.userid], function(error, results2) {
										...
										if (results2[0].level >= results[0].required) {
													...
													connection.query('UPDATE users SET gold = gold + ?, credits = credits + ?, mithril = mithril + ? WHERE id = ?', [results[0].gold, results[0].credits, results[0].mithril, req.session.userid], function(error) {
														...
														connection.query('INSERT INTO users_milestones SET ?', {
															...
															connection.commit(function(error) {
																...
																connection.release();
																res.end(menu + '<center>Completed milestone and received <b>' + results[0].gold + '</b> gold, <b>' + results[0].credits + '</b> credits, and <b>' + results[0].mithril + '</b> mithril</center>' + milestoneCurrency);
															});
														});
													});
												});
											...
											return res.end(menu + '<center>Your level is not high enough for this milestone.</center>' + milestoneCurrency);
										}
									});
								} else {
									connection.query('SELECT kills FROM users WHERE id = ?', [req.session.userid], function(error, results2) {
										...
										if (results2[0].kills >= results[0].required) {
											...
											return res.end(menu + '<center>Your kill count is not high enough for this milestone.</center>' + milestoneCurrency);
										}
									});
								}
							});
