...
module.exports = function(req, res) {
	if (req.session.loggedIn) {
		if (req.body.x == 1) {
			if (Number.isInteger(parseInt(req.body.thing)) && parseInt(req.body.thing) > 0 && parseInt(req.body.thing) <= 1000) {
				connection.query('SELECT credits, pcredits FROM users WHERE id = ?', [req.session.userid], function(error, results) {
					if (error) {
						return res.end('error');
					}
					if (results[0].credits >= parseInt(req.body.thing) * 2 && results[0].pcredits >= parseInt(req.body.thing) * 2) {
						var timeToAdd = function() {
							if ((Math.floor(Date.now() / 1000) + parseInt(req.body.thing) * 60) - parseInt(gsettings[5].value) >= parseInt(req.body.thing) * 60) {
								return Math.floor(Date.now() / 1000) + parseInt(req.body.thing) * 60;
							} else {
								return parseInt(gsettings[5].value) + parseInt(req.body.thing) * 60;
							}
						}
						connection.getConnection(function(error, connection) {
							...
							connection.beginTransaction(function(error) {
								...
								connection.query('UPDATE users SET credits = credits - ?, pcredits = pcredits - ? WHERE id = ?', [parseInt(req.body.thing) * 2, parseInt(req.body.thing) * 2, req.session.userid], function(error) {
									if (error) {
										connection.rollback();
										...
										connection.query('SELECT * FROM settings', function(error, results) {
											...
											connection.query('INSERT INTO chat_messages SET ?', {
												...
												connection.commit(function(error) {
													...
													connection.release();
													res.end('Purchase success![BREAK]' + (results[0].credits - parseInt(req.body.thing) * 2));
												});
											});
										});
									});
			...
			if (Number.isInteger(parseInt(req.body.thing))) {
				...
				res.end('Invalid package.[BREAK]' + results[0].uCredits);
			}
			...
			connection.query('SELECT reputation, gender FROM users WHERE id = ?', [req.session.userid], function(error, results) {
				if (Number.isInteger(parseInt(req.body.thing)) && req.body.thing <= reputation_ranks.length && results[0].reputation >= reputation_ranks[req.body.thing - 1].reputation) {
					connection.query('UPDATE users SET reprank = ? WHERE id = ?', [req.body.thing, req.session.userid], function(error) {
						...
						if (results[0].gender == 1) {
							var rankDisplay = reputation_ranks[req.body.thing - 1].nameM;
						} else {
							var rankDisplay = reputation_ranks[req.body.thing - 1].nameF;
						}
						res.send('status1[BREAK]' + rankDisplay);
					});
				} else {
					res.status(400).send('not enough rep');
				}
			});
				...
				connection.query('SELECT COUNT(*) AS count FROM users_items WHERE user = ?', [req.session.userid], function(error, results) {
					...
					if (Number.isInteger(parseInt(req.body.thing))) {
						if (shops[shopitems[req.body.thing - 1].shop - 1].merge == 1) {
							...
							connection.query('SELECT gold, credits FROM users WHERE id = ?', [req.session.userid], function(error, results) {
								if (error) {
									return res.end('error');
								}
									...
									connection.getConnection(function(error, connection) {
										...
										connection.beginTransaction(function(error) {
											...
											async.forEachOf(funcP2, function(result, d, callback) {
												var amountReq = funcP2[d].quantity;
												var neededReq = funcP2[d].requirement;
												connection.query('SELECT * FROM users_items WHERE item = ? AND user = ? FOR UPDATE', [neededReq, req.session.userid], function(error, results) {
													...
													if (results.length > 0 && results[0].quantity > amountReq) {
														connection.query('UPDATE users_items SET quantity = quantity - ? WHERE item = ? AND user = ?', [amountReq, neededReq, req.session.userid], function(error, results) {
															if (error) {
																connection.rollback();
																return res.end('Not enough merge materials.[BREAK]' + userData[0].gold + '[BREAK]' + userData[0].credits);
															}
														...
														connection.query('DELETE FROM users_items WHERE item = ? AND user = ?', [neededReq, req.session.userid], function(error, results) {
															if (error) {
																connection.rollback();
																return res.end('Not enough merge materials.[BREAK]' + userData[0].gold + '[BREAK]' + userData[0].credits);
															}
												...
												connection.query('SELECT * FROM users_items WHERE item = ? AND user = ? FOR UPDATE', [shopitems[req.body.thing - 1].item, req.session.userid], function(error, results) {
													...
													if (results.length > 0 && items[shopitems[req.body.thing - 1].item - 1].stack >= results[0].quantity) {
														...
														connection.query('UPDATE users_items SET quantity = quantity + 1 WHERE item = ? AND user = ?', [shopitems[req.body.thing - 1].item, req.session.userid], function(error, results) {
															if (error) {
																connection.rollback();
																return res.end('server error.');
															}
															if (z == 1) {
																connection.query('UPDATE users SET credits = credits - ? WHERE id = ?', [items[shopitems[req.body.thing - 1].item - 1].cost, req.session.userid], function(error, results) {
																	if (error) {
																		connection.rollback();
																		return res.end('server error.');
																	}
																...
																connection.query('UPDATE users SET gold = gold - ? WHERE id = ?', [items[shopitems[req.body.thing - 1].item - 1].cost, req.session.userid], function(error, results) {
																	if (error) {
																		connection.rollback();
																		return res.end('server error.');
																	}
											...
											connection.beginTransaction(function(error) {
												...
												connection.query('SELECT * FROM users_items WHERE item = ? AND user = ?', [items[shopitems[req.body.thing - 1].item - 1].id, req.session.userid], function(error, results) {
													...
													if (results[0].quantity >= items[shopitems[req.body.thing - 1].item - 1].stack) {
														...
														connection.query('UPDATE users_items SET quantity = quantity + 1 WHERE item = ? AND user = ?', [items[shopitems[req.body.thing - 1].item - 1].id, req.session.userid], function(error, results) {
															if (error) {
																connection.rollback();
																return res.end('server error.');
															}
