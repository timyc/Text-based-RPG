		...
		if (req.body.x == 'rankings') {
			if (req.body.y == 1) {
				connection.query('SELECT username, level FROM users ORDER BY level DESC LIMIT 50', function(error, results) {
					if (error) {
						...
						res.write('<center><h3>Rankings</h3></center>');
						res.write('<center><input type="button" class="btn btn-outline-light border-white" value="Level" onclick="openpage(2, 1)"> <input type="button" class="btn btn-outline-light border-white" value="Kills" onclick="openpage(2, 2)"> <input type="button" class="btn btn-outline-light border-white" value="Reputation" onclick="openpage(2, 3)"></center>');
			...
			connection.query('SELECT reprank, reputation, gender FROM users WHERE id = ?', [req.session.userid], function(error, results) {
				if (error) {
					...
					if (results[0].gender == 1) {
						for (var x = 0; x < reputation_ranks.length; x++) {
			...
			connection.query('SELECT * FROM shops_items WHERE shop = ?', [req.body.y], function(error, results) {
				if (error) {
					...
					var creditsDisplay;
					res.write('<center><h3>' + shops[req.body.y - 1].name + '</h3></center>')
					...
					for (var x = 0; x < results.length; x++) {
						...
						if (shops[req.body.y - 1].merge == 0) {
							if (items[results[x].item - 1].type == 'artifact') {
								res.write('<tr><td><font color="' + color + '" title="' + items[results[x].item - 1].description + '">' + items[results[x].item - 1].name + '</font></td>' + '<td>' + items[results[x].item - 1].health + '</td>' + '<td>' + items[results[x].item - 1].attack + '</td>' + '<td>' + items[results[x].item - 1].defense + '</td>' + '<td>' + items[results[x].item - 1].accuracy + '</td>' + '<td>' + items[results[x].item - 1].evasion + '</td>' + '<td>' + creditsDisplay + '</td>' + '<td class="text-center"><input type="button" onclick="buy(21, ' + results[x].id + ')" value="Purchase" class="btn btn-outline-light border-white moveBtn"></td>' + '</tr>');
							} else {
								res.write('<tr><td><font color="' + color + '" title="' + items[results[x].item - 1].description + '">' + items[results[x].item - 1].name + '</font></td>' + '<td>N/A</td>' + '<td>N/A</td>' + '<td>N/A</td>' + '<td>N/A</td>' + '<td>N/A</td>' + '<td>' + creditsDisplay + '</td>' + '<td class="text-center"><input type="button" onclick="buy(21, ' + results[x].id + ')" value="Purchase" class="btn btn-outline-light border-white moveBtn"></td>' + '</tr>');
							}
						} else {
							var mergeItems = '';
							var arrayIt = JSON.parse(JSON.stringify((itemreqs)));
							var funcP2 = arrayIt.filter(function(special) {
								return special.item == results[x].item;
							});
							for (var y = 0; y < funcP2.length; y++) {
								mergeItems += '<li>' + funcP2[y].quantity + ' ' + items[funcP2[y].requirement - 1].name + '</li>';
							}
							if (items[results[x].item - 1].type == 'artifact') {
								res.write('<tr><td><font color="' + color + '" title="' + items[results[x].item - 1].description + '">' + items[results[x].item - 1].name + '</font></td>' + '<td>' + items[results[x].item - 1].health + '</td>' + '<td>' + items[results[x].item - 1].attack + '</td>' + '<td>' + items[results[x].item - 1].defense + '</td>' + '<td>' + items[results[x].item - 1].accuracy + '</td>' + '<td>' + items[results[x].item - 1].evasion + '</td>' + '<td><ul style="margin-bottom:0px !important"><li>' + creditsDisplay + '</li>' + mergeItems + '</ul></td>' + '<td class="text-center"><input type="button" onclick="buy(21, ' + results[x].id + ')" value="Merge" class="btn btn-outline-light border-white moveBtn"></td>' + '</tr>');
							} else {
								res.write('<tr><td><font color="' + color + '" title="' + items[results[x].item - 1].description + '">' + items[results[x].item - 1].name + '</font></td>' + '<td>N/A</td>' + '<td>N/A</td>' + '<td>N/A</td>' + '<td>N/A</td>' + '<td>N/A</td>' + '<td><ul style="margin-bottom:0px !important"><li>' + creditsDisplay + '</li>' + mergeItems + '</ul></td>' + '<td class="text-center"><input type="button" onclick="buy(21, ' + results[x].id + ')" value="Merge" class="btn btn-outline-light border-white moveBtn"></td>' + '</tr>');
							}
						}
					}
			...
			connection.query('SELECT * FROM faq', function(error, results) {
				if (error) {
					...
					for (var x = 0; x < results.length; x++) {
						res.write('<div class="card bg-dark text-white" style="display:block;text-align:left;border-radius:0.25rem;border: 1px solid rgba(0,0,0,.125);margin-bottom:20px"><h4 class="card-header">' + results[x].question + '</h4><div class="card-body">');
						res.write('<p class="card-text">' + results[x].answer + '</p>');
						res.write('</div></div>');
					}
					res.end();
				}
			...
			connection.query('SELECT * FROM news ORDER BY id DESC', function(error, results) {
				if (error) {
					...
					for (var x = 0; x < results.length; x++) {
						res.write('<div class="card bg-dark text-white" style="display:block;text-align:left;border-radius:0.25rem;border: 1px solid rgba(0,0,0,.125);margin-bottom:20px"><h4 class="card-header">' + results[x].title + '</h4><div class="card-body">');
						res.write('<p class="card-text">' + results[x].content + '</p>');
						res.write('<p class="card-text">By ' + results[x].author + ' on ' + results[x].date + '</p>');
						res.write('</div></div>');
					}
					res.end();
				}
			...
			res.write('<center><h3>Commands</h3></center>');
			...
			res.write('<p>/g message - guild chat<br />/w username message - whisper chat<br />/p message - pub chat<br />/m message - market chat<br />/roll - rolls 1d100 and is used with other channels (IE /p /roll to roll in pub)<br />/me message - message displayed in role-play form<br />@ExactUsername - highlights message for user</p><hr><p>Press "W" to switch to simple theme<br />Press "H" to hide the game header</p>');
			res.write('</div></div></div><div class="col"></div></div>');
			res.end();
		} else if (req.body.x == 'rules') {
			res.write('<center><h3>Chat rules</h3></center>');
			...
			if (req.body.y == 1) {
				...
				connection.query('SELECT pcredits FROM users WHERE id = ?', [req.session.userid], function(error, results) {
					...
					res.write('<center><h3>Purchase global boost</h3></center>');
					res.write('<center>' + menu + '</center>');
					res.write('<div class="row"><div class="col">');
					res.write('<br /><center>You have <b>' + results[0].pcredits + '</b> purchased credits, obtained through <b>Purchase</b></center><br />');
				...
				connection.query('SELECT reputation, expboost, goldboost, inventory, equipslots, autos, credits, pcredits FROM users WHERE id = ?', [req.session.userid], function(error, results) {
					...
					res.write('<center><h3>Purchase misc bonuses</h3></center>');
					res.write('<center>' + menu + '</center><br />');
					res.write('<center>You have <b>' + results[0].credits + '</b> credits.</center>');
					...
					res.write('<center>You have <b>' + results[0].pcredits + '</b> purchased credits (shows how many normal credits can be used for upgrades below.)</center><br />');
				...
				connection.query('SELECT * FROM users_purchases WHERE claimed = 0 AND user = ?', [req.session.userid], function(error, results) {
					...
					res.write('<center><h3>Retrieve Purchases</h3></center>');
					res.write('<center>' + menu + '</center>');
					...
					for (var pur = 0; pur < results.length; pur++) {
			...
			res.write('<center><h3>Code redemption</h3></center>');
			res.write('<center><p>Find codes on our Discord!</p></center>');
			res.write('<center><p id="redeemResponse"></p></center>');
			res.write('<div class="row"><div class="col"></div><div class="col-8">');
			res.write('<div class="input-group mb-3"><input type="text" autocomplete="off" id="redeem" class="form-control border-white" placeholder="Enter code" style="background-color: rgba(0, 0, 0, 0.3);"><div class="input-group-append"><input type="button" id="redeembutton" class="btn btn-outline-light border-white" value="Redeem" onclick="redeemCode();"></div></div>');
			res.write('</div><div class="col"></div></div>');
			res.end();
		} else if (req.body.x == 'mail') {
			var menu = '<a href="javascript:openpage(11, 1)" class="btn btn-outline-light border-white">Inbox</a> <a href="javascript:openpage(11, 3)" class="btn btn-outline-light border-white">Send Mail</a>';
			connection.query('SELECT level, gold FROM users WHERE id = ?', [req.session.userid], function(error, userGold) {
				...
				if (req.body.y == 1) {
					connection.query('SELECT users_mail.id AS id, users_mail.title AS title, users_mail.sender AS sender, FROM_UNIXTIME(UNIX_TIMESTAMP(users_mail.sent), "%Y-%m-%d %h:%i:%s") AS sent, users_mail.read AS `read`, users.id AS userid, users.username AS username FROM users_mail, users WHERE users_mail.receiver = ? AND users.id = users_mail.sender ORDER BY users_mail.id DESC LIMIT 25', [req.session.userid], function(error, results) {
						...
						res.write('<center><h3>Mail Inbox (' + results.length + '/25)</h3></center>');
						res.write('<center>' + menu + '</center>');
					...
					connection.query('SELECT users_mail.*, users.id AS userid, users.username AS username FROM users_mail, users WHERE users_mail.id = ? AND users_mail.receiver = ? AND users.id = users_mail.sender', [req.body.z, req.session.userid], function(error, results) {
						...
						if (results.length == 1) {
							connection.query('UPDATE users_mail SET `read` = 1 WHERE id = ?', [req.body.z], function(error) {
								...
								res.write('<center><h3>Read Mail</h3></center>');
								res.write('<center>' + menu + '</center>');
								res.write('<div class="row"><div class="col"></div><div class="col-8">');
								res.write('<center><h4>' + results[0].title + '</h4></center>');
								res.write('<center><b>From ' + results[0].username + '</b></center>');
								res.write('<p>' + results[0].content + '</p>');
								if (results[0].gold > 0 && results[0].retrieved == 0) {
									res.write('<p>Attached: ' + toCoin(results[0].gold) + ' gold <a href="javascript:mail(3, ' + results[0].id + ')" class="text-light">[Claim]</a></p>');
								} else if (results[0].gold > 0 && results[0].retrieved == 1) {
									res.write('<p>Attached: ' + toCoin(results[0].gold) + ' gold <s>Already claimed</s></p>');
								}
								res.write('</div><div class="col"></div></div>[BREAK]' + userGold[0].gold);
								res.end();
							});
						} else {
							res.end('<center>HEY, THAT MAIL IS NOT FOR YOU!</center>[BREAK]' + userGold[0].gold);
						}
					});
				} else if (req.body.y == 3) {
					res.write('<center><h3>Send Mail</h3></center>');
					res.write('<center>' + menu + '</center>');
					if (userGold[0].level >= 500) {
						...
						res.write('<div class="input-group mb-3"><div class="input-group-prepend"><span class="input-group-text">Gold (10% taxed)</span></div><input type="number" min="0" class="form-control" value="0" id="toGold"></div>');
						res.write('<input type="button" class="btn btn-outline-light border-white" value="Send message (100 gold)" onclick="mail(1);">')
						res.write('</div></div>[BREAK]' + userGold[0].gold);
						res.end();
					} else {
						res.end('<br /><center>The New Government does not allow recruits to send messages until they are at least level 500.</center>[BREAK]' + userGold[0].gold);
					}
				} else {
					res.end('?');
				}
			...
			var menu = '<a href="javascript:openpage(12, 1, 0)" class="btn btn-outline-light border-white">Credits</a> <a href="javascript:openpage(12, 2, 0)" class="btn btn-outline-light border-white">Sold</a> <a href="javascript:openpage(12, 3, 0)" class="btn btn-outline-light border-white">Retrieve</a> | <a href="javascript:openpage(12, 4)" class="btn btn-outline-light border-white">Sell Credits</a>';
			if (req.body.y == 1 && Number.isInteger(parseInt(req.body.z))) {
				connection.query('SELECT * FROM market WHERE type = "credits" AND status = "selling" AND expires >= NOW() LIMIT ' + (req.body.z * 10) + ', 10', function(error, results) {
					...
					res.write('<center><h3>Credits Market</h3></center>');
					res.write('<center>' + menu + '</center>');
					...
					for (var listing = 0; listing < results.length; listing++) {
					...
					if (results.length == 10 && req.body.z == 0) {
						res.write('<center><a href="javascript:openpage(12, 1, ' + (req.body.z + 1) + ')" class="text-light">Next</a></center>');
					} else if (results.length == 10 && req.body.z > 0) {
						res.write('<center><a href="javascript:openpage(12, 1, ' + (req.body.z - 1) + ')" class="text-light">Prev</a> | <a href="javascript:openpage(12, 1, ' + (req.body.z + 1) + ')" class="text-light">Next</a></center>');
					} else if (results.length < 10 && req.body.z > 0) {
						res.write('<center><a href="javascript:openpage(12, 1, ' + (req.body.z - 1) + ')" class="text-light">Prev</a></center>');
					}
					res.end();
				});
			} else if (req.body.y == 2 && Number.isInteger(parseInt(req.body.z))) {
				connection.query('SELECT * FROM market WHERE status = "sold" AND user = ? LIMIT ' + (req.body.z * 10) + ', 10', [req.session.userid], function(error, results) {
					...
					res.write('<center><h3>Sold Items</h3></center>');
					res.write('<center>' + menu + '</center>');
					...
					res.write('</tbody></table></div><div class="col"></div></div><hr>');
					if (results.length == 10 && req.body.z == 0) {
						res.write('<center><a href="javascript:openpage(12, 2, ' + (req.body.z + 1) + ')" class="text-light">Next</a></center>');
					} else if (results.length == 10 && req.body.z > 0) {
						res.write('<center><a href="javascript:openpage(12, 2, ' + (req.body.z - 1) + ')" class="text-light">Prev</a> | <a href="javascript:openpage(12, 2, ' + (req.body.z + 1) + ')" class="text-light">Next</a></center>');
					} else if (results.length < 10 && req.body.z > 0) {
						res.write('<center><a href="javascript:openpage(12, 2, ' + (req.body.z - 1) + ')" class="text-light">Prev</a></center>');
					}
					res.end();
				});
				...
				connection.query('SELECT * FROM market WHERE status = "selling" AND user = ? AND expires < NOW() LIMIT ' + (req.body.z * 10) + ', 10', [req.session.userid], function(error, results) {
					...
					res.write('<center><h3>Retrieve Unsold Items</h3></center>');
					res.write('<center>' + menu + '</center>');
					...
					res.write('</tbody></table></div><div class="col"></div></div><hr>');
					if (results.length == 10 && req.body.z == 0) {
						res.write('<center><a href="javascript:openpage(12, 3, ' + (req.body.z + 1) + ')" class="text-light">Next</a></center>');
					} else if (results.length == 10 && req.body.z > 0) {
						res.write('<center><a href="javascript:openpage(12, 3, ' + (req.body.z - 1) + ')" class="text-light">Prev</a> | <a href="javascript:openpage(12, 3, ' + (req.body.z + 1) + ')" class="text-light">Next</a></center>');
					} else if (results.length < 10 && req.body.z > 0) {
						res.write('<center><a href="javascript:openpage(12, 3, ' + (req.body.z - 1) + ')" class="text-light">Prev</a></center>');
					}
					res.end();
				});
			} else if (req.body.y == 4) {
				connection.query('SELECT pcredits FROM users WHERE id = ?', [req.session.userid], function(error, results) {
					...
					res.write('<center><h3>Sell Credits</h3></center>');
					res.write('<center>' + menu + '</center><br />');
					res.write('<center>You have <b>' + results[0].pcredits + '</b> sellable credits, obtained through <b>Purchase</b></center><br />');
					res.write('<center><b class="text-warning">Listed credits will be UNRETRIEVABLE for 12 hours!</b></center><br />');
					res.write('<div class="row"><div class="col"></div><div class="col-8">');
			...
			connection.query('SELECT COUNT(DISTINCT ipaddress) AS count FROM users WHERE level >= 100 AND referrer = ? AND (created between DATE_FORMAT(NOW() ,"%Y-%m-01") AND NOW())', [req.session.userid], function(error, results) {
				...
				res.write('<center><h3>Referral Info</h3></center>');
				res.write('<center>Your referral link: <b class="text-success">https://tolandsunknown.com/?ref=' + req.session.userid + '</b></center>');
				res.write('<center>Your referrals this month: <b>' + results[0].count + '</b></center><br />');
			...
			connection.query('SELECT access, username, reprank, gender FROM users WHERE lastonline > (NOW() - INTERVAL 1 HOUR) ORDER BY access DESC, username ASC', function(error, results) {
				...
				var shownRank;
				var authority;
				res.write('<center><h3>Who is online?</h3></center>');
				res.write('<center>Below is the list of users who logged in or fought a mob in the last hour (doesn\'t count lurkers)</center>');
				...
				for (var x = 0; x < results.length; x++) {
					...
					res.write('<tr><td style="padding:0px"><a href="javascript:profile(&quot;' + results[x].username + '&quot;);" class="text-light">' + authority + ' ' + results[x].username + ' ' + shownRank + '</a></td></tr>');
				}
				res.write('</tbody></table></div><div class="col"></div></div>');
				res.end();
			});
