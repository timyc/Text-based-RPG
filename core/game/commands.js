	...
	if (req.session.loggedIn) {
		if (req.body.x == 'online') {
			...
			if (req.session.access >= 60) {
				connection.query('SELECT * FROM monsters', function(error, results, fields) {
					if (error) {
						return res.end('error');
					}
					...
					connection.query('SELECT * FROM items', function(error, results, fields) {
						...
						items = results;
						connection.query('SELECT * FROM monsters_drops', function(error, results, fields) {
							if (error) {
								return res.end('error');
							}
							...
							connection.query('SELECT * FROM settings', function(error, results, fields) {
								...
								connection.query('SELECT * FROM locations', function(error, results, fields) {
									...
									locations = results;
									connection.query('SELECT * FROM locations_places', function(error, results, fields) {
										if (error) {
											return res.end('error');
										}
										console.log('[MYSQL] Reloaded location places');
										locPlaces = results;
										connection.query('SELECT * FROM beasts', function(error, results, fields) {
											if (error) {
												return res.end('error');
											}
											console.log('[MYSQL] Reloaded beasts');
											beasts = results;
											connection.query('SELECT * FROM story', function(error, results, fields) {
												if (error) {
													return res.end('error');
												}
												console.log('[MYSQL] Reloaded story');
												story = results;
												connection.query('SELECT * FROM reputation_ranks', function(error, results, fields) {
													if (error) {
														return res.end('error');
													}
													...
													connection.query('SELECT * FROM shops', function(error, results, fields) {
														...
														shops = results;
														connection.query('SELECT * FROM shops_items', function(error, results, fields) {
															if (error) {
																return res.end('error');
															}
															console.log('[MYSQL] Reloaded shop items');
															shopitems = results;
															connection.query('SELECT * FROM items_requirements', function(error, results, fields) {
																if (error) {
																	return res.end('error');
																}
...
exports.toPlace = function(req, res) {
	if (req.session.loggedIn) {
		connection.query('SELECT * FROM users WHERE id = ?', [req.session.userid], function(error, userData) {
			...
			var placeToGo = req.body.x;
			var arrayIt = JSON.parse(JSON.stringify((locPlaces)));
			var funcP1 = arrayIt.filter(function(special) {
			...
			if (funcP1.length == 1 && userData[0].level >= locations[placeToGo - 1].level) {
				if (locations[placeToGo - 1].reqitem > 0) {
					connection.query('SELECT * FROM users_items WHERE item = ? AND user = ?', [locations[funcP1[0].location - 1].reqitem, req.session.userid], function(error, results) {
						...
						if (results.length == 1) {
							var funcP2 = (funcP1[0].function).split(' ')[1];
							var funcP3 = arrayIt.filter(function(special) {
								return special.location == funcP2 &&
									special.function == 'enter ' + funcP1[0].location;
							});
							connection.query('UPDATE users SET location = ?, locationx = ?, locationy = ? WHERE id = ?', [funcP2, funcP3[0].locationx, funcP3[0].locationy, req.session.userid], function(error, results) {
								if (error) {
									return error;
								} else {
									res.send('<span id="bgChange" style="display:none;">' + locations[funcP2 - 1].id + '</span>' + locations[funcP2 - 1].name + ' <a rel="popover" data-trigger="hover click" data-placement="bottom" title="' + locations[funcP2 - 1].description + '">[?]</a><br /><input type="button" onclick="toPlace(' + funcP2 + ')" value="[Portal to ' + locations[funcP1[0].location - 1].name + ']" class="blankBtn"><br />' + funcP3[0].locationx + ' , ' + funcP3[0].locationy);
								}
							});
						} else {
							res.send('<span id="bgChange" style="display:none;">' + locations[userData[0].location - 1].id + '</span><span class="text-danger">Required item: </span>' + items[locations[placeToGo - 1].reqitem - 1].name + '<br />' + locations[userData[0].location - 1].name + ' <a rel="popover" data-trigger="hover click" data-placement="bottom" title="' + locations[userData[0].location - 1].description + '">[?]</a><br /><input type="button" onclick="toPlace(' + placeToGo + ')" value="[Portal to ' + locations[placeToGo - 1].name + ']" class="blankBtn"><br />' + userData[0].locationx + ' , ' + userData[0].locationy);
						}
					});
				} else {
					var funcP2 = (funcP1[0].function).split(' ')[1];
						...
						connection.query('SELECT * FROM users_items WHERE user = ? AND item = ?', [locations[results[0].location - 1].reqitem, req.session.userid], function(error, results2) {
							...
							if (results2.length == 1) {
								if (userData[0].gold >= 1500000) {
									connection.query('UPDATE users SET gold = gold - 1500000, location = ?, locationx = ?, locationy = ? WHERE id = ?', [results[0].location, results[0].locationx, results[0].locationy, req.session.userid], function(error, results4) {
										if (error) {
											return res.end('database error');
										}
...
exports.teleportLoc = function(req, res) {
	if (req.session.loggedIn) {
		if (Number.isInteger(parseInt(req.body.x)) && req.body.x <= locations.length) {
					...
					if (locations[req.body.x - 1].reqitem > 0) {
						connection.query('SELECT * FROM users_items WHERE item = ? AND user = ?', [locations[req.body.x - 1].reqitem, req.session.userid], function(error, results) {
							...
							if (results.length == 1 && userData[0].gold >= 1000000) {
								connection.query('UPDATE users SET gold = gold - 1000000, location = ?, locationx = 0, locationy = 0 WHERE id = ?', [req.body.x, req.session.userid], function(error, results) {
									if (error) {
										return res.end('database error');
									}
			...
			res.end('undefined location[BREAK]' + userData[0].gold);
		}
	} else {
		res.end('not logged in');
	}
...
exports.mail = function(req, res) {
	if (req.session.loggedIn) {
		connection.query('SELECT level, gold FROM users WHERE id = ?', [req.session.userid], function(error, userData) {
			...
			if (req.body.x == 1 && userData[0].level >= 500 && Number.isInteger(parseInt(req.body.v)) && parseInt(req.body.v) >= 0 && req.session.gold >= parseInt(req.body.v) + 100) {
				connection.query('SELECT id FROM users WHERE username = ?', [req.body.y], function(error, results) {
					if (error) {
						return res.end('database error 1');
					}
					if (results.length == 1) {
						connection.query('SELECT id FROM users_mail WHERE receiver = ?', [results[0].id], function(error, mailTotal) {
							if (error) {
								return res.end('error');
							}
								...
								connection.getConnection(function(error, connection) {
									...
									connection.beginTransaction(function(error) {
										...
										connection.query('INSERT INTO users_mail SET ?', {
											...
											title: req.body.z,
											content: req.body.w,
											gold: Math.floor(parseInt(req.body.v) * 0.9)
										}, function(error) {
											if (error) {
												connection.rollback();
												return res.end('database error 2');
											}
											connection.query('UPDATE users SET gold = gold - ? WHERE id = ?', [parseInt(req.body.v) + 100, req.session.userid], function(error) {
												if (error) {
													connection.rollback();
												...
												connection.commit(function(error) {
													...
													connection.release();
													req.session.gold -= (parseInt(req.body.v) + 100);
													res.end('Successfully sent mail to ' + req.body.y + '![BREAK]' + (userData[0].gold - (parseInt(req.body.v) + 100)));
												});
											});
										});
									});
								...
								connection.query('UPDATE users SET gold = gold + ? WHERE id = ?', [results[0].gold, req.session.userid], function(error) {
									...
									connection.query('UPDATE users_mail SET retrieved = 1 WHERE id = ?', [req.body.y], function(error) {
										...
										connection.commit(function(error) {
											...
											connection.release();
											req.session.gold += results[0].gold;
											res.end('Successfully claimed ' + toCoin(userData[0].gold + results[0].gold) + ' coins![BREAK]' + (userData[0].gold + results[0].gold));
										});
									});
								});
							});
		...
		if (req.session.lastBotcheck && Date.now() - req.session.lastBotcheck <= 90000) {
			if (req.body.response == answer) {
				connection.query('UPDATE users SET warns = warns - 1 WHERE id = ?', [req.session.userid], function(error) {
					...
					res.end('<center>Verified</center>');
				});
			} else {
				req.session.lastBotcheck = null;
				res.end('<center>Wrong answer</center>');
			}
		} else {
			req.session.lastBotcheck = null;
			res.end('<center>Too late</center>');
		}
	} else {
		res.end('not logged in');
	}
